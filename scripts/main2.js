	...
	(function(){
		...
		while(
			container.hasChildNodes()){container.removeChild(container.firstChild
			);
		}
		...
		if(canvas){ctx.drawImage(canvas,width-x,0);
		...
		var scrollCanvas=function(){viewport.scrollLeft++;
		if(viewport.scrollLeft>=totalWidth-width){freshCanvas();
		}};

		var drawTheLine=function(){ctx.beginPath();ctx.moveTo(x,y);
		x+=pan;
		...
		var tilt=function(axes){if(axesPrev){
			for(var i=0;i<axes.length;i++){
				var delta=axes[i]-axesPrev[i];
		if(Math.abs(delta)>Math.abs(deflection)){deflection=delta;
		}}}
